...
def create_mark(request, payload: dict, strict_daily_pair: bool = True) -> Tuple[EventoAsistencia, str]:
    ...
    if target_empleado_id:
        if not (getattr(request.user, "is_superadmin", False) or request.user.has_role("SUPERADMIN") or request.user.has_role("ADMIN_RRHH")):
    ...
    if geocerca:
        ...
        # If geofence exists and gps is provided, block outside.
        if dentro is False:
