...
def _turno_segments(turno) -> List[Tuple[Any, Any]]:
    """Devuelve segmentos [(start_time, end_time), ...] para el turno.

    Compatibilidad:
    - Siempre incluye (turno.hora_inicio, turno.hora_fin) si existen.
    - Si Turno.dias_semana es dict con {segments:[..., {start,end}]}, agrega
      un 2do segmento.
    """
        ...
        if isinstance(raw_segs, list) and len(raw_segs) >= 2 and isinstance(raw_segs[1], dict):
            ...
            try:
                ...
                if isinstance(s2, str):
                    s2 = _dt.strptime(s2[:5], "%H:%M").time()
                if isinstance(e2, str):
                    e2 = _dt.strptime(e2[:5], "%H:%M").time()
            except Exception:
                s2, e2 = None, None
            if s2 and e2:
                segs.append((s2, e2))
...
def get_state(empresa_id, empleado_id, today: Optional[date] = None, strict_daily_pair: bool = True) -> AttendanceState:
    ...
    # Segmentos del turno: 1 jornada => 2 eventos (IN+OUT), 2 jornadas => 4 eventos (IN+OUT+IN+OUT)
    segs = _turno_segments(turno)
    max_events = (len(segs) * 2) if segs else 2
    ...
    # Validar secuencia del día y determinar la próxima acción.
    # Secuencia esperada:
    #  - Jornada simple: IN, OUT
    #  - Doble jornada:  IN, OUT, IN, OUT
    expected = ["check_in", "check_out"] * (2 if max_events == 4 else 1)

    # strict: limitar número de eventos por día
    if strict_daily_pair and len(events) >= max_events:
        return AttendanceState(
            next_code=None,
            next_label="Jornada completada",
            done=True,
            reason=(
                "Ya registraste entrada y salida hoy." if max_events == 2
                else "Ya registraste las 2 jornadas de hoy."
            ),
    ...
    # Verificar consistencia de los eventos existentes
    for idx, ev in enumerate(events):
        if strict_daily_pair and idx >= max_events:
            break
        exp = expected[idx]
        exp_id = check_in_id if exp == "check_in" else check_out_id
        if str(ev.tipo) != str(exp_id):
            return AttendanceState(
                next_code=None,
    ...
    # Labels más claros cuando hay doble jornada
    step = len(events) + 1  # 1..max_events
    jornada_n = 1 if step <= 2 else 2
    label_base = "Registrar Entrada" if next_code == "check_in" else "Registrar Salida"
    next_label = f"{label_base} {jornada_n}" if max_events == 4 else label_base
...
def _validate_time_window(turno, next_code: str, now_local: datetime, hoy: date, step: int = 1) -> None:
    ...
    # Turno times can be null. Para doble jornada se usan los segmentos.
    segs = _turno_segments(turno)
    if not segs:
        return

    # step: 1 IN1, 2 OUT1, 3 IN2, 4 OUT2
    seg_idx = 0
    if len(segs) >= 2 and step >= 3:
        seg_idx = 1
    start_t, end_t = segs[seg_idx]
...
def create_mark(request, payload: dict, strict_daily_pair: bool = True) -> Tuple[EventoAsistencia, str]:
    ...
    # Step del día (1..4):
    #  1=Entrada 1, 2=Salida 1, 3=Entrada 2, 4=Salida 2
    day_events_count = EventoAsistencia.objects.filter(
        empresa_id=empresa_id,
        empleado_id=empleado_id,
        registrado_el__date=hoy,
    ).count()
    step = int(day_events_count) + 1
    ...
    _validate_time_window(turno, next_code, now_local, hoy, step=step)
