...
def _save_attendance_photo(base64_data: str, empresa_id, empleado_id) -> str | None:
    ...
    Se usa en la marcación (webcam).
    Hace normalización para evitar imágenes negras/corruptas por payloads incompletos.
    """
    if not base64_data:
        return None

    b64 = str(base64_data).strip()
    ext = "jpg"
    ...
    # Normalización: algunas implementaciones envían espacios en lugar de +
    ...
    # Evita guardar archivos vacíos o muy pequeños (típico de capturas fallidas)
...
def _rebuild_jornada(empresa_id, empleado_id, fecha: date, turno: Turno | None, regla: ReglaAsistencia | None):
    ...
    for ev in eventos:
        if str(ev.tipo) == str(check_in_id):
            ...
            if open_in is not None and ev.registrado_el and ev.registrado_el > open_in:
                ...
                pairs.append({
                    "in": open_in.isoformat() if open_in else None,
                    "out": ev.registrado_el.isoformat() if ev.registrado_el else None,
    ...
    if open_in is not None:
        now = timezone.now()
        if now > open_in:
            partial = int((now - open_in).total_seconds() // 60)
            minutos_trabajados += partial
            pairs.append({
                "in": open_in.isoformat() if open_in else None,
                "out": None,
                "minutes": partial,
    ...
    # Para doble jornada: la referencia de inicio es la 1ra jornada; la referencia de fin
    # es el fin de la última jornada.
    start_ref = getattr(turno, "hora_inicio", None) if turno else None
    end_ref = getattr(turno, "hora_fin", None) if turno else None

    if turno and isinstance(getattr(turno, "dias_semana", None), dict):
        raw = turno.dias_semana
        segs = raw.get("segments") or []
        if isinstance(segs, list) and len(segs) >= 2 and isinstance(segs[1], dict):
            end2 = segs[1].get("end")
            try:
                if isinstance(end2, str):
                    end_ref = datetime.strptime(end2[:5], "%H:%M").time()
            except Exception:
                pass

    if turno and first_in and start_ref:
    ...
    if turno and last_out and end_ref:
    ...
    detalles = {
        "pairs": pairs,
        "segments": (turno.dias_semana.get("segments") if turno and isinstance(getattr(turno, "dias_semana", None), dict) else None),
    }
