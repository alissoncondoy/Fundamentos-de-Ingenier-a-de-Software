...
from ...models import UnidadOrganizacional, Puesto, Empleado, Rol, Turno
...
class AjaxRoles(_AjaxOptionsBase):
    def get(self, request):
        ...
        if not (request.user.has_role("SUPERADMIN") or getattr(request.user, "is_superadmin", False)):
            qs = qs.exclude(nombre__iexact="SUPERADMIN")
        if empresa_id:
...
class AjaxTurnos(_AjaxOptionsBase):
    """Opciones de Turnos por empresa.

    Se usa para combos dependientes (p.ej. Asignación de turno). A propósito NO filtra por empleado
    para permitir asignar cualquier turno disponible de la empresa.
    """
