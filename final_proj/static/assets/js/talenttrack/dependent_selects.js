(function () {
  ...
  const urls = {
    ...
    turnos: cfg.getAttribute('data-turnos-url')
  };

  const empresaSel = document.querySelector('select[name="empresa"]');
  const unidadSel = document.querySelector('select[name="unidad"]');
  // En algunos formularios (p.ej. UnidadOrganizacional) el select dependiente se llama "padre"
  // pero necesita el mismo catálogo de "unidades" filtrado por empresa.
  const padreSel = document.querySelector('select[name="padre"]');
  ...
  const turnoSel = document.querySelector('select[name="turno"]');
  ...
  async function reloadDependent() {
    ...
    const deps = [unidadSel, padreSel, puestoSel, managerSel, empleadoSel, rolSel, turnoSel].filter(Boolean);
    if (!empresaId) {
      deps.forEach(el => {
    ...
    try {
      ...
      if (padreSel) setDisabled(padreSel, true);
      ...
      if (turnoSel) setDisabled(turnoSel, true);

      const tasks = [];
      // Unidades (se usa para select "unidad" y/o "padre")
      if (unidadSel || padreSel) tasks.push(fetchOptions(urls.unidades, empresaId));
      ...
      if (turnoSel && urls.turnos) tasks.push(fetchOptions(urls.turnos, empresaId));
      else tasks.push(Promise.resolve([]));

      const [unidades, puestos, managers, empleados, roles, turnos] = await Promise.all(tasks);
      ...
      if (padreSel) {
        fillSelect(padreSel, unidades, 'Seleccione unidad padre…');
        setDisabled(padreSel, false);
      }
      if (puestoSel) {
      ...
      if (turnoSel) {
        fillSelect(turnoSel, turnos, 'Seleccione turno…');
        setDisabled(turnoSel, false);
      }
    } catch (err) {
      console.error(err);
      const deps2 = [unidadSel, puestoSel, managerSel, empleadoSel, rolSel, turnoSel].filter(Boolean);
      deps2.forEach(el => {
