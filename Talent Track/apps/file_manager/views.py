...
# ===== TARGET (lo que tu código "cliente" quiere usar) =====
class FilePreviewer(ABC):
    @abstractmethod
    def preview(self, file_path: str) -> str:
        """Devuelve un texto 'preview' del archivo."""
        raise NotImplementedError


# ===== ADAPTEE (lo existente / legacy) =====
# Ojo: su "interfaz" NO coincide con el Target (no se llama preview)
class LegacyCsvConverter:
    def csv_to_text(self, csv_file_path: str) -> str:
        # reutiliza tu lógica actual
        return convert_csv_to_text(csv_file_path)


# ===== ADAPTER (traduce Target -> Adaptee) =====
class CsvPreviewAdapter(FilePreviewer):
    def __init__(self, legacy_csv_converter: LegacyCsvConverter):
        self.legacy = legacy_csv_converter

    def preview(self, file_path: str) -> str:
        # Traducción: preview(...) -> csv_to_text(...)
        return self.legacy.csv_to_text(file_path)
        ...
        if os.path.isfile(file_path):
            try:
                ...
                if extension.lower() == '.csv':
                    previewer = CsvPreviewAdapter(LegacyCsvConverter())
                    csv_text = previewer.preview(file_path)
